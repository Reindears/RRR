...
async def cb_handlers(c: Client, cb: "types.CallbackQuery"):
    if cb.data == "showSettings":
        ...
        editable = await c.send_message(cb.message.chat.id, text="Now send me new file name", reply_to_message_id=cb.reply_to_message.message_id, reply_markup = ForceReply())
